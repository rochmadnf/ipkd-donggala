...
@section('content')
    ...
    <form id="formYear" action="{{ config('app.url') }}/attachments" method="GET" class="flex flex-row items-center gap-x-4">
        <button id="prevYear" type="button" class="rotate-180 hover:opacity-75">&#10132;</button>
        <input id="seasonYear" data-init-value="{{ $initSeason }}" name="season" type="tel" maxlength="4"
            minlength="4" min="1998"
            class="block w-20 rounded-md border border-b-[3px] p-2 text-center font-bold tracking-wide outline-none"
            value="{{ $initSeason }}" />
        <button id="nextYear" type="button" class="hover:opacity-75">&#10132;</button>
