...
@section('content')
    ...
    <form id="formYear" action="{{ route('attachments') }}" method="GET" class="flex flex-row items-center gap-x-4">
        ...
        <input data-init-value="{{ $initSeason }}" name="season" id="seasonYear" type="tel" maxlength="4"
    ...
    <div id="file-list" class="md:max-h-[320px] space-y-4 overflow-y-auto p-4">
        @forelse ($files as $file)
            <x-card-file :path="asset_storage($file['filepath'])" :name="$file['filename']" :upload_date="$file['uploaded_at']->translatedFormat('d M Y')" :order="$file['order']" />
        @empty
